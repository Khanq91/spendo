...
class HabitDetector {
  ...
  /// Chạy full analysis, nhưng throttle: bỏ qua nếu đã analyze trong 24h qua.
  /// Gọi khi user mở RemindersScreen.
  static Future<void> analyze() async {
    if (await _analyzedRecently()) return;
    await _runAnalysis();
  }

  /// Kiểm tra xem có bất kỳ habit nào được analyze trong 24h gần nhất không.
  /// Dùng row mới nhất trong detected_habits để làm mốc.
  static Future<bool> _analyzedRecently() async {
    final row = await db.getOptional(
      'SELECT analyzed_at FROM detected_habits ORDER BY analyzed_at DESC LIMIT 1',
    );
    if (row == null) return false;

    final lastStr = row['analyzed_at'] as String?;
  ...
  static Future<void> _runAnalysis() async {
    ...
    for (final entry in groups.entries) {
      ...
      final dates = occurrences.map((e) => e.date).toList()
  ...
  static String _normalize(String note) => note.toLowerCase().trim();
